...
- **Image files**: Stored as `/app/data/image/{id}-albedo.png` and `/app/data/image/{id}-normals.png`
- **Configuration**: Stored as `/app/data/image/{id}-config.yml`
- **Manifest file**: Generated as `/app/data/iiif/manifest/{id}.json`
- **Image tiles**: Generated in directories `/app/data/iiif/image/{id}-albedo/` and `/app/data/iiif/image/{id}-normals/`
...
- Content persists across `docker compose down` and container rebuilds
